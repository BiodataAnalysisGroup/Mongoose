...
def anndata_from_outputs(model, outputs):
    ...
    adata = ad.AnnData(fused_latents if isinstance(fused_latents, np.ndarray) else fused_latents.cpu().numpy())
    if hasattr(model, 'class_weights'):
        adata.obs["predicted_label"] = model.label_encoder.inverse_transform(
            predictions.tolist() if isinstance(predictions, np.ndarray) else predictions.cpu().tolist()
        )
    else:
