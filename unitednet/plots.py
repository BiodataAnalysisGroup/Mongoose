...
def color_cm(
    ...
    for i in range(NUM_COLORS):
        color.append(
            matplotlib.colors.to_hex(cm(1.0 * i / NUM_COLORS))
        )  # color will now be an RGBA tuple
    return color


def type_relevance_chord_plot(
    shap_values,
    p_fe,
    p_fe_idx,
    p_l_less,
    ...
    colors_mods = [
        color_cm(potential_coloarmaps[ii], len(c_p_fe))
        for ii, c_p_fe in enumerate(p_fe)
    ]
    scaler = MinMaxScaler()
    all_feature_num = sum([len(pp) for pp in p_fe])
    all_type_features = {}
    all_type_features_scores = {}
    aggregated_shap_values = {}
    aggregated_shap_values_scaled = {}
    for ls_t in all_less_type:
        ...
        for uni_idx, tp in enumerate(uni_sub):
            ...
            all_type_features_scores[pr_ty_dict[tp]] = {}
            aggregated_shap_values = []
            aggregated_shap_values_scaled[pr_ty_dict[tp]] = {}
            for ii in range(len(p_fe)):
                # here we used all the shap feature for downstream calculation
                aggregated_shap = np.mean(np.abs(shap_values[tp][ii]), axis=0)[
                    p_fe_idx[ii]
                ].reshape(-1, 1)
                ft = scaler.fit_transform(aggregated_shap).squeeze()
                c_v.append(ft)
                ...
                if uni_idx == 0:
                    ...
                    node_angles.append(
                        (np.linspace(start, end, len(p_fe_idx[ii]))).astype(int)[::+1]
                        % 360
                    )
                    start = end
                    node_colors.append(colors_mods[ii])
                all_type_features[pr_ty_dict[tp]][ii] = good_fe[good_fe != ""]
                all_type_features_scores[pr_ty_dict[tp]][ii] = ft[good_fe != ""]
                aggregated_shap_values.append(aggregated_shap[good_fe != ""])
            shap_all_modalities = np.concatenate(aggregated_shap_values)
            aggregated_shap_values_scaled[pr_ty_dict[tp]] = scaler.fit_transform(shap_all_modalities).squeeze()
        ...
        node_angles_s = np.hstack(
            [
                (np.linspace(45, 315, len(names_s[: -len(uni_sub)]))).astype(int)[::+1]
                % 360,
                node_angles[-len(uni_sub) :],
            ]
        )
        ...
        if make_plot:
            fig, axes = plot_connectivity_circle(
                c_v[good_in_index],
                names_s,
                indices=(c_v_in_s, c_v_out_s),
                node_width=node_width,
                node_linewidth=0,
                node_colors=node_colors_s,
                facecolor="white",
                textcolor="k",
                linewidth=linewidth,
                node_angles=node_angles_s,
            ...
            if save_fig:
                fig.savefig(
                    f"./feature_relevance_figures/{technique}_type_{ls_t}_only_show_good.pdf"
            )
    return all_type_features, all_type_features_scores, aggregated_shap_values_scaled


def markers_chord_plot(
    adatas_all, predict_label, predict_label_anno, major_dict, subset_feature=True
):
...
def feature_relevance_chord_plot(
    shap_values_0_1,
    unique_ct,
    var_names_all,
    all_type_features,
    ...
    scaler = MinMaxScaler()
    direction_dict = {
        0: int(direction.split("to")[0]),
        1: int(direction.split("to")[-1]),
    }
    colors_mods = [
        color_cm(potential_coloarmaps[direction_dict[ii]], len(c_p_fe))
        for ii, c_p_fe in enumerate(var_names_all)
    ]
    ...
    for ct in unique_ct:
        ...
        for ii_m, v_n in enumerate(var_names_all):
            _m, _i, _ = np.intersect1d(
                v_n, all_type_features[ct][direction_dict[ii_m]], return_indices=True
            )
        ...
        if thres != None:
            ...
            c_v = c_v.ravel()
            in_names, _in_idx, _ = np.intersect1d(
                var_names_all[0], cur_marker[0], return_indices=True
            )
            out_names, _out_idx, _ = np.intersect1d(
                var_names_all[1], cur_marker[1], return_indices=True
            )
        ...
        for ii, nm in enumerate(io_names):
            ...
            node_angles.append(
                (np.linspace(start, end, len(io_names[ii][io_names[ii] != ""]))).astype(
                    int
                )[::+1]
                % 360
            )
        ...
        if make_plot:
            ...
            if save_fig:
                fig.savefig(
                    f"./feature_relevance_figures/{technique}_feature_{ct}_only_show_good_{direction}.pdf"
            )
