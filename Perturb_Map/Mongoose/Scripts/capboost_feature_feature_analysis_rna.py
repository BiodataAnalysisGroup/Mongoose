#!/usr/bin/env python3
"""
Feature-to-Feature Cross-Modal Prediction Analysis for RNA CapBoost UnitedNet
(using *discovered_cluster* labels instead of ground-truth phenotypes)

This script performs cross-modal prediction analysis between RNA, Activity, and Niche modalities
and calculates feature-to-feature relevance using SHAP values.

Usage:
    python capboost_feature_feature_analysis_rna.py \
        --dataset_id KP2_1 \
        --shap_dir ../Task1_JGI/shap_analysis_capboost_rna_20251128_123456
"""
...
def load_shap_results(shap_dir):
    """Load SHAP analysis results from RNA CapBoost analysis"""
    shap_dir = Path(shap_dir)

    print(f"Looking for RNA CapBoost SHAP results in: {shap_dir}")
    ...
    # Load the RNA CapBoost SHAP results
    results = {}
    files_to_load = {
        'all_type_features': 'all_type_features_perturbmap_capboost_rna.pkl',
        'scores': 'scores_perturbmap_capboost_rna.pkl',
        'aggregated_shap': 'aggregated_shap_perturbmap_capboost_rna.pkl',
        'shap_values': 'shap_values_perturbmap_capboost_rna.pkl'
    }
...
def find_latest_capboost_model(base_path="../Models/UnitedNet", dataset_id=None):
    """Find the most recent RNA CapBoost model file"""
    import glob

    search_patterns = []
    if dataset_id:
        search_patterns.extend([
            f"{base_path}/perturbmap_rna_activity_niche_capboost_*/model_*_{dataset_id}_*.pkl",
    ...
    print(f"Found RNA CapBoost models:")
...
def prepare_capboost_data_for_cross_modal(data_path, dataset_id):
    """
    Prepare RNA CapBoost data (RNA+Activity+Niche) for cross-modal analysis.
    ...
    # Load RNA CapBoost data files (RNA+Activity+Niche)
    files_to_load = {
    ...
    for key in ['adata_rna_train', 'adata_activity_train', 'adata_niche_train',
    ...
    print(f"Prepared RNA CapBoost data shapes:")
...
def compute_cross_modal_shap_capboost(model, adatas_all, source_modality, target_modality,
    ...
    """Compute SHAP values for cross-modal prediction in RNA CapBoost model"""

    modality_names = ["RNA", "ACTIVITY", "NICHE"]

    print(f"Computing SHAP: {modality_names[source_modality]} -> {modality_names[target_modality]}")

    shap_file = Path(output_dir) / f'shap_values_capboost_rna_{source_modality}_{target_modality}.pkl'
...
def str_to_mod_id_capboost(x):
    """Convert modality string/int to RNA CapBoost modality ID"""
    s = str(x).lower()
    if s in ("0", "rna", "r"): return 0
    if s in ("1", "activity", "act", "a"): return 1
    if s in ("2", "niche", "n"): return 2
    try:
        return int(s)
    except:
        raise KeyError(f"Unknown RNA CapBoost modality key: {x}")


def normalize_all_type_features_capboost(all_type_features):
    """Normalize cluster keys and modality subkeys for RNA CapBoost"""
    ...
    for k, sub in all_type_features.items():
        ...
        # Handle modality subkeys for RNA CapBoost
        normalized_sub = {}
...
def feature_feature_relevance_capboost(shap_values_dict, all_type_features, var_names_all,
                                       source_id, target_id, technique="perturbmap_capboost_rna",
                                       make_plot=False):
    """Compute feature-to-feature relevance between RNA CapBoost modalities"""
...
def extract_feature_feature_values_capboost(shap_values_dict, all_type_features, var_names_all,
                                            combinations, technique="perturbmap_capboost_rna"):
    """Extract feature-feature values for all RNA CapBoost combinations"""

    id2modality = {0: "RNA", 1: "ACTIVITY", 2: "NICHE"}
    rows = []
    ...
    print("Processing RNA CapBoost feature-feature relationships...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Feature-to-feature cross-modal prediction analysis for RNA CapBoost UnitedNet (RNA+Activity+Niche)",
    ...
    parser.add_argument('--shap_dir', type=str, required=True,
                        help='Directory containing RNA CapBoost SHAP analysis results')
    parser.add_argument('--model_path', type=str, default=None,
                        help='Path to trained RNA CapBoost model (auto-detected if not provided)')
    ...
    parser.add_argument('--combinations', type=str, nargs='+',
                        ...
                        help='Modality combinations to analyze (0=RNA, 1=Activity, 2=Niche)')
    ...
    if args.timestamp:
        ...
        output_dir = Path(args.output_dir) / f"capboost_rna_cross_modal_analysis_{timestamp}"
    else:
        output_dir = Path(args.output_dir) / "capboost_rna_cross_modal_analysis"

    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"RNA CapBoost Cross-Modal Analysis")
    print(f"Results will be saved to: {output_dir}")
    print(f"Modalities: RNA(0), Activity(1), Niche(2)")

    try:
        # Load SHAP results from RNA CapBoost analysis
        print("\n=== Loading RNA CapBoost SHAP Results ===")
        ...
        if all_type_features is None:
            raise ValueError("all_type_features not found in RNA CapBoost SHAP results")

        # Find and load RNA CapBoost model
        ...
        print(f"Loading RNA CapBoost model from: {model_path}")
        with open(model_path, 'rb') as f:
            model = pickle.load(f)

        # Prepare RNA CapBoost data (NOW USING discovered_cluster)
        print("\n=== Preparing RNA CapBoost Data (using discovered_cluster) ===")
        ...
        print("\n=== Extracting RNA CapBoost Feature-Feature Relationships ===")
        ...
        # Save results
        feature_feature_file = output_dir / 'capboost_rna_feature_feature_importance_rna_activity_niche.csv'
        df.to_csv(feature_feature_file, index=False)

        print(f"\n=== RNA CapBoost Analysis Complete ===")
        ...
        summary = {
            ...
            'output_files': {
                ...
                'cross_modal_shap': [str(output_dir / f'shap_values_capboost_rna_{s}_{t}.pkl')
                                     for s, t in combinations]
            }
        }

        summary_file = output_dir / 'capboost_rna_analysis_summary.json'
        ...
        if len(df) > 0:
            ...
            ax1.set_title('Distribution of Feature-Feature Relationships by Direction (RNA Model)')
            ...
            plt.tight_layout()
            viz_file = output_dir / 'capboost_rna_feature_analysis_summary.png'
        ...
        print(f"\nError during RNA CapBoost analysis: {e}")
        import traceback
        traceback.print_exc()
        return 1
