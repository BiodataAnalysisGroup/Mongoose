#!/usr/bin/env python3
"""
Cluster-Phenotype Correspondence Analysis for RNA+Activity+Niche Model (FIXED v2)

CHANGES FROM ORIGINAL:
- Uses RNA modality instead of Flux
- Properly handles phenotype extraction to avoid numpy array comparison errors
- Converts all phenotype values to strings before sorting
- Uses pandas crosstab to handle non-contiguous cluster labels correctly

Usage:
    python analyze_cluster_phenotype_correspondence_rna.py --model_path <path_to_model>
"""
...
def prepare_data_for_shap(adata_dict):
    ...
    rna_all = sc.concat([adata_dict['adata_rna_train'], 
                         adata_dict['adata_rna_test']], 
                        join='inner')
    activity_all = sc.concat([adata_dict['adata_activity_train'], 
                              adata_dict['adata_activity_test']], 
                             join='inner')
    niche_all = sc.concat([adata_dict['adata_niche_train'], 
                           adata_dict['adata_niche_test']], 
                          join='inner')
    ...
    for adata in [rna_all, activity_all, niche_all]:
        if 'phenotypes' in adata.obs.columns:
            ...
            if hasattr(phenotypes, 'categories'):
                ...
                phenotypes = [str(x) if not isinstance(x, (list, np.ndarray)) else str(x[0]) 
                             for x in phenotypes]
            
            # Filter out NaN, None, 'nan', 'None' strings
            phenotypes_clean = []
            for p in phenotypes:
    ...
    if all_phenotypes:
        ...
        for adata in [rna_all, activity_all, niche_all]:
            if 'phenotypes' in adata.obs.columns:
                ...
                # Also store the cleaned string phenotypes
...
def load_model_and_get_predictions(model_path, data_path, dataset_id):
    """Load model and get predictions - RNA+Activity+Niche version"""
    ...
    # Prepare data using FIXED function
    ...
    try:
        ...
        # Fallback: use KMeans on combined features
        combined_features = np.concatenate([
            adatas_all[0].X,  # RNA
            adatas_all[1].X,  # Activity  
            adatas_all[2].X   # Niche
        ], axis=1)
        
        # Determine number of clusters from model config or default to 6
        n_clusters = 6
    ...
    # Get original phenotypes - properly extract as strings
    ...
    if hasattr(phenotypes_raw, 'categories'):
        ...
        original_phenotypes = np.array([str(x) if not isinstance(x, (list, np.ndarray)) else str(x[0]) 
...
def create_correspondence_analysis(predict_label, original_phenotypes, output_dir):
    ...
    print(f"\n{'='*60}")
    print(f"CORRESPONDENCE ANALYSIS SUMMARY")
    print(f"{'='*60}")
    ...
    # Create mapping for phenotypes to numeric codes (for metrics)
    ...
    # 1. Build correspondence table using pandas crosstab - handles non-contiguous labels!
    ...
    # Rename columns to have Cluster_ prefix
    ...
    # 2. Correspondence Tables - percentages
    correspondence_pct = correspondence_df.div(correspondence_df.sum(axis=1), axis=0) * 100
    
    print(f"\n{'='*60}")
    print(f"CORRESPONDENCE TABLE (Raw Counts)")
    print(f"{'='*60}")
    print(correspondence_df.to_string())
    
    print(f"\n{'='*60}")
    print(f"CORRESPONDENCE TABLE (Percentages)")
    print(f"{'='*60}")
    ...
    for phenotype in unique_phenotypes:
        ...
        phenotype_recovery[phenotype] = {
            ...
            'distribution': {f'Cluster_{int(c)}': int(cnt) for c, cnt in zip(unique, counts)}
        }
    ...
    for orig_idx, pred_idx in zip(row_ind, col_ind):
        best_mapping[f'Cluster_{unique_pred_clusters[pred_idx]}'] = unique_phenotypes[orig_idx]
    ...
    bars2 = ax2.bar(range(len(phenotypes)), recoveries, color='lightcoral', 
    ...
    for i, phenotype in enumerate(unique_phenotypes):
        ...
        ax1.bar(cluster_names, values, bottom=bottom1, label=phenotype, 
               color=phenotype_colors[i], edgecolor='white', linewidth=0.5)
        bottom1 += values
    ...
    for i, cluster in enumerate(cluster_names):
        ...
        ax2.bar(unique_phenotypes, values, bottom=bottom2, label=cluster, 
    ...
    print(f"\n{'='*60}")
    print(f"DETAILED PHENOTYPE RECOVERY")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze correspondence between predicted clusters and original phenotypes (RNA version)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    
    parser.add_argument('--model_path', type=str, required=True,
                       help='Path to trained RNA+Activity+Niche model pickle file')
    ...
    parser.add_argument('--output_dir', type=str, default='../Analysis/Cluster_Phenotype_Correspondence',
    ...
    output_dir = Path(args.output_dir) / f"correspondence_analysis_rna_{timestamp}"
    
    print(f"\n{'='*60}")
    print(f"CLUSTER-PHENOTYPE CORRESPONDENCE ANALYSIS (RNA Version)")
    print(f"{'='*60}")
    ...
    try:
        ...
        results = create_correspondence_analysis(predict_label, original_phenotypes, output_dir)
