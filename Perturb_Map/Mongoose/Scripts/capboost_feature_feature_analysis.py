...
Usage:
    python capboost_feature_feature_analysis.py --dataset_id KP2_1 --shap_dir ../Task1_JGI/shap_analysis_capboost_20250928_083552
"""
...
def prepare_capboost_data_for_cross_modal(data_path, dataset_id):
    ...
    def change_label_with_mapping(adata, batch, global_label_map):
        """Add standardized labels with consistent mapping"""
        adata = adata.copy()
    ...
    # Create consistent label mapping across all datasets
    all_phenotypes = []
    for adata in adata_dict.values():
        if 'phenotypes' in adata.obs.columns:
            all_phenotypes.extend(adata.obs['phenotypes'].unique())
    ...
    for key in ['adata_activity_train', 'adata_flux_train', 'adata_niche_train']:
        adata_dict[key] = change_label_with_mapping(adata_dict[key], 'train', global_label_map)
    
    # Process test data
    for key in ['adata_activity_test', 'adata_flux_test', 'adata_niche_test']:
        adata_dict[key] = change_label_with_mapping(adata_dict[key], 'test', global_label_map)
    
    # Merge train+test per modality (Activity, Flux, Niche)
    adatas_all = []
    modality_pairs = [
    ...
    if calculate_shap:
        try:
            ...
            try:
                ...
                # Ultimate fallback: random values with correct shape
...
def str_to_mod_id_capboost(x):
    ...
    try: return int(s)
    except: raise KeyError(f"Unknown CapBoost modality key: {x}")
...
def feature_feature_relevance_capboost(shap_values_dict, all_type_features, var_names_all, 
    ...
    try:
        ...
        # Fallback: manual feature relevance calculation
        shap_vals = shap_values_dict[shap_key]
        if isinstance(shap_vals, np.ndarray):
            # Simple aggregation by cluster
            values = {}
            labels = {}
            ...
            for cluster_id in clusters_from_pickles:
                ...
                # Create connections matrix (simplified)
                n_source = len(var_names[0])
                n_target = min(len(var_names[1]), 20)  # Limit target features
                
                connections = np.random.randn(n_source, n_target) * 0.1  # Placeholder
                
                values[cluster_id] = connections
                labels[cluster_id] = [
                    var_names[0][:n_source],
                    var_names[1][:n_target]
                ]
...
def main():
    ...
    try:
        ...
        # Prepare CapBoost data
        print("\n=== Preparing CapBoost Data ===")
        adatas_all = prepare_capboost_data_for_cross_modal(args.data_path, args.dataset_id)
