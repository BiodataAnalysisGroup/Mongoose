#!/usr/bin/env python3
"""
UnitedNet PerturbMap — Activity + Niche (2 Modalities)

Goal
-----
Train UnitedNet with only 2 modalities: Activity and Niche.
Optimized for studying GenKI perturbation effects across regulatory (Activity) and spatial (Niche) modalities.
Based on the working 3-modality capboost script but adapted for 2 modalities.

Models will be saved to: ../Models/UnitedNet/ (default)

CLI example
-----------
python uni_training_activity_niche_v2.py \
  --data_path ../Data/UnitedNet/input_data \
  --dataset_id KP2_1 \
  --train_epochs 40 --finetune_epochs 20 \
  --lr 5e-4 \
  --scale_activity --scale_niche \
...
def _normalize_train_return(train_ret, model):
    ...
    if isinstance(train_ret, dict):
        ...
        return (loss, train_ret.get("acc") or train_ret.get("accuracy"),
                train_ret.get("ari"), train_ret.get("nmi"))
...
class MetricHistory:
    ...
    def merge_finetune(self, lists, r2_matrix_per_epoch=None):
        ...
        if nf > n0:
            ...
            for k in ["train_acc","train_nmi","train_ari","train_loss","ft_acc","ft_nmi","ft_ari","ft_loss","r2_mean"]:
        ...
        if nf > 0:
            ...
            for i in range(nf):
                ...
                if r2_matrix_per_epoch is not None:
                    ...
                    if r2m is not None:
                        try:
                            self.hist["r2_mean"][idx] = float(np.nanmean(np.array(r2m, dtype=float)))
                        except Exception:
...
def ensure_dense_float32(adata):
    if sp.issparse(adata.X):
        X = adata.X.tocsr(); X.sort_indices()
...
# ---------- config builder (Activity + Niche) ----------

def get_activity_niche_config():
    """Two‑modality config optimized for Activity + Niche with enhanced Niche branch."""
    return {
        'train_batch_size': 16,
        ...
        # Enhanced CMP weights for Activity→Niche prediction
        'train_loss_weight': {
            'cross_modal_prediction': 5.0,     # Emphasize CMP
            'within_modal_reconstruction': 2.5,
            'clustering': 1.0,
            'contrastive': 1.2,
            'discriminator': 1.0,
            'generator': 1.0,
        ...
        'finetune_loss_weight': {
            'cross_modal_prediction': 4.0,     # Still elevated in FT
            'within_modal_reconstruction': 2.5,
            'clustering': 1.2,
            'contrastive': 1.2,
            'discriminator': 1.0,
        ...
        # Optional per‑edge CMP weights (if your UnitedNet version supports it)
        'cmp_edge_weights': None,  # e.g., {"Activity->Niche": 2.0}

        'lr': 0.0005,
        'checkpoint': 20,
        'n_head': 10,
        'noise_level': [0.0, 0.02],   # Activity, Niche
        'fuser_type': 'WeightedMean',

        'encoders': [
            # Activity encoder 
            {'input': 2504, 'hiddens':[512, 256, 128, 64], 'output': 64,
             'use_biases':[True]*5, 'dropouts':[0.05, 0.05, 0.0, 0.0, 0.0],
             'activations':['relu','relu','relu','relu',None],
             'use_batch_norms':[False]*5, 'use_layer_norms':[False,False,False,False,True],
             'is_binary_input': False},
            # Niche encoder (CapBoost): wider + earlier LayerNorm
            {'input': 2504, 'hiddens':[768, 256, 128, 64], 'output': 64,
             'use_biases':[True]*5,
             'dropouts':[0.05, 0.03, 0.0, 0.0, 0.0],
             'activations':['relu','relu','relu','relu',None],
             'use_batch_norms':[False, False, False, False, False],
             'use_layer_norms':[True, False, False, False, True],  # LN at first hidden & output
             'is_binary_input': False},
        ],
        ...
        'decoders': [
            # Activity decoder
            {'input': 64, 'hiddens':[64, 128, 256, 512], 'output': 2504,
             'use_biases':[True]*5, 'dropouts':[0.0, 0.0, 0.05, 0.05, 0.0],
             'activations':['relu','relu','relu','relu',None],
             'use_batch_norms':[False]*5, 'use_layer_norms':[True,False,False,False,False]},
            # Niche decoder (CapBoost): slightly less dropout on deeper layers
            {'input': 64, 'hiddens':[64, 128, 256, 768], 'output': 2504,
             'use_biases':[True]*5, 'dropouts':[0.0, 0.0, 0.03, 0.03, 0.0],
             'activations':['relu','relu','relu','relu',None],
             'use_batch_norms':[False]*5, 'use_layer_norms':[True,False,False,False,False]},
        ],

        'discriminators': [
            {'input': 2504, 'hiddens':[512, 256, 128], 'output': 1,
             'use_biases':[True]*4, 'dropouts':[0.1, 0.1, 0.0, 0.0],
             'activations':['relu','relu','relu','sigmoid'],
             'use_batch_norms':[False]*4, 'use_layer_norms':[False,False,False,True]},
            {'input': 2504, 'hiddens':[512, 256, 128], 'output': 1,
             'use_biases':[True]*4, 'dropouts':[0.1, 0.1, 0.0, 0.0],
             'activations':['relu','relu','relu','sigmoid'],
             'use_batch_norms':[False]*4, 'use_layer_norms':[False,False,False,True]},
        ...
        'projectors': {
            ...
            'use_biases': [True], 'dropouts': [0.0],
            'activations': ['relu'], 'use_batch_norms': [False],
            'use_layer_norms': [True],
        },

        'clusters': {
            'input': 100, 'hiddens': [], 'output': 4,  # Will be updated from args.n_clusters
            'use_biases': [False], 'dropouts': [0.0],
            'activations': [None], 'use_batch_norms': [False],
            'use_layer_norms': [False],
        },
    }

def main():
    parser = argparse.ArgumentParser(description="UnitedNet Activity + Niche (2 modalities)")
    
    # Data arguments
    parser.add_argument('--data_path', required=True, help="Path to input data directory")
    parser.add_argument('--dataset_id', required=True, help="Dataset identifier (e.g., KP2_1)")
    
    # Training arguments
    parser.add_argument('--train_epochs', type=int, default=40, help="Training epochs")
    parser.add_argument('--finetune_epochs', type=int, default=20, help="Finetune epochs")
    parser.add_argument('--train_batch_size', type=int, default=16, help="Training batch size")
    parser.add_argument('--finetune_batch_size', type=int, default=16, help="Finetune batch size")
    parser.add_argument('--lr', type=float, default=5e-4, help="Learning rate")
    parser.add_argument('--n_clusters', type=int, default=4, help="Number of output clusters")
    parser.add_argument('--noise_level', nargs=2, type=float, default=[0.0, 0.02], 
                       help="Noise levels for [Activity, Niche]")
    
    # Model arguments
    parser.add_argument('--technique', default='activity_niche_2mod', help="Technique identifier")
    parser.add_argument('--device', default='auto', help="Device to use (auto, cpu, cuda, cuda:0, cuda:1, etc.)")
    parser.add_argument('--seed', type=int, default=42, help="Random seed")
    
    # Data processing
    parser.add_argument('--scale_activity', action='store_true', help="Apply z-score scaling to Activity")
    parser.add_argument('--scale_niche', action='store_true', help="Apply z-score scaling to Niche")
    
    # Cross-modal prediction enhancement
    parser.add_argument('--cmp_edge_activity_to_niche', type=float, default=0.0,
                       help="Edge-specific CMP weight for Activity→Niche prediction")
    
    # Output arguments
    parser.add_argument('--output_base', default='../Models/UnitedNet', help="Base output directory")
    parser.add_argument('--model_suffix', default='', help="Optional model filename suffix")
    parser.add_argument('--timestamp', action='store_true', help="Add timestamp to output directory")
    
    # Training mode
    parser.add_argument('--train_model', action='store_true', default=True, help="Train the model")
    parser.add_argument('--load_only', action='store_true', help="Only load existing model (skip training)")
    parser.add_argument('--verbose', action='store_true', help="Verbose training output")
    
    args = parser.parse_args()
    
    # Override train_model if load_only specified
    if args.load_only:
    ...
    if args.device == 'auto':
        ...
        try:
            ...
            if device_id >= torch.cuda.device_count():
                print(f"⚠️  CUDA device {device_id} not available (only {torch.cuda.device_count()} devices), falling back to cuda")
                args.device = 'cuda'
        except (ValueError, IndexError):
            print(f"⚠️  Invalid CUDA device specification '{args.device}', falling back to cuda")
            args.device = 'cuda'

    out_dir = setup_output_directory(args.output_base, args.technique, args.timestamp)
    root_save_path = str(out_dir)
    print(f"Output directory: {out_dir}")

    # Build config
    config = get_activity_niche_config()
    ...
    config['lr'] = args.lr
    config['clusters']['output'] = args.n_clusters  # Update clusters output instead of n_clusters
    config['noise_level'] = [float(args.noise_level[0]), float(args.noise_level[1])]
    ...
    for name, a, b in [("Activity", activity_tr, activity_te), ("Niche", niche_tr, niche_te)]:
        if not a.var_names.equals(b.var_names):
            ...
            a = a[:, common].copy(); b = b[:, a.var_names].copy()
            if name == "Activity":
    ...
    print("\n=== Building Train & All Sets (Activity + Niche) ===")
    activity_all = concat_train_test(activity_tr, activity_te)
    niche_all    = concat_train_test(niche_tr,    niche_te)

    train_list = [ensure_dense_float32(x.copy()) for x in [activity_tr, niche_tr]]
    all_list   = [ensure_dense_float32(x.copy()) for x in [activity_all, niche_all]]

    # Overwrite I/O dims from data
    print("\n=== Configuring Model I/O from data ===")
    modality_names = ['Activity', 'Niche']
    ...
    # Train / load
    print(f"\n=== {'Training' if args.train_model else 'Loading'} UnitedNet (Activity + Niche) ===")
    model = UnitedNet(str(out_dir), device=args.device, technique=config)

    # Ensure the internal save_path is set to avoid 'None/...' saves
    ...
    # Save outputs
    model_fn = f"model_perturbmap_activity_niche_{args.dataset_id}"
    ...
    summary = {
        ...
        "scaling": {"activity": bool(args.scale_activity), "niche": bool(args.scale_niche)},
    ...
    print("\n=== Notes ===")
    print("- Enhanced Niche branch capacity for handling spatial averaging complexity")
    print("- Optional Activity→Niche cross-modal prediction enhancement")
    print("- Enable --scale_niche and --scale_activity for better stability")
    print("- Perfect for studying GenKI regulatory vs spatial perturbation effects")
...
"""
Alternative usage with sys.argv (for notebook/programmatic execution):

import sys
sys.argv = [
    "uni_training_activity_niche_v2.py",     # 2 modalities: Activity (2504 features) + Niche (2504 features)
    "--data_path", "../Data/UnitedNet/input_data",
    "--dataset_id", "KP2_1",
    "--train_epochs", "40",
    "--finetune_epochs", "20",
    "--n_clusters", "4",
    "--noise_level", "0.0", "0.02",  # 2 values for Activity, Niche
    "--model_suffix", "activity_niche",
    "--timestamp",
    "--verbose",
    "--device", "cuda:0",  # Now supports specific CUDA devices: auto, cpu, cuda, cuda:0, cuda:1, etc.
    "--scale_activity",
    "--scale_niche",
    "--cmp_edge_activity_to_niche", "2.0",
    # "--output_base", "../Models/UnitedNet",  # Optional (this is now the default)
]
from uni_training_activity_niche_v2 import main
main()
"""