#!/usr/bin/env python3
"""
Cluster-Phenotype Correspondence Analysis for CapBoost Model (FIXED v2)
...
Usage:
    python analyze_cluster_phenotype_correspondence_fixed.py --model_path <path_to_model>
"""
...
def prepare_data_for_shap(adata_dict):
    ...
    activity_all = sc.concat([adata_dict['adata_activity_train'], 
                             ...
                             join='inner')
    flux_all = sc.concat([adata_dict['adata_flux_train'], 
                          adata_dict['adata_flux_test']], 
                         join='inner')
...
def load_model_and_get_predictions(model_path, data_path, dataset_id):
    """Load model and get predictions - FIXED VERSION"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze correspondence between predicted clusters and original phenotypes (FIXED v2)",
    ...
    print(f"\n{'='*60}")
    print(f"CLUSTER-PHENOTYPE CORRESPONDENCE ANALYSIS")
    print(f"{'='*60}")
