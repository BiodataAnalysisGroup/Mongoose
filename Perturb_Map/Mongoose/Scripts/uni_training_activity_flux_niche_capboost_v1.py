#!/usr/bin/env python3
"""
UnitedNet PerturbMap — Activity + Flux + Niche (CapBoost v1)

Goal
-----
Same as the baseline 3‑modality script, but with **branch‑specific capacity & regularization** for the Niche arm and
optionally **edge‑specific CMP weights** that emphasize Activity→Niche prediction.

What changes vs v1
------------------
1) **Niche encoder widened/deepened**: 1500 → [768, 256, 128, 64] → 64 (vs 512/256/128/64). Early LayerNorm added.
2) **Niche decoder slightly less dropout** to make reconstruction easier/stable.
3) **Global CMP weight higher** (train: 5.0, finetune: 4.0).
4) **Optional edge‑specific CMP weights** via `technique['cmp_edge_weights']`, e.g. {('Activity','Niche'): 2.0}.
   This requires your UnitedNet build to read and use this dict. If unsupported, it will be ignored without errors.
5) **Sets model.save_path** to avoid `None/train_epoch_*.pt` messages.

CLI example
-----------
python uni_training_activity_flux_niche_capboost_v1.py \
...
# ---------- config builder (CapBoost) ----------

def get_capboost_config():
    """Three‑modality config with beefed‑up Niche branch and higher CMP weights."""
    return {
        'train_batch_size': 16,
        ...
        # Stronger CMP; slightly higher recon/contrastive
        'train_loss_weight': {
        ...
        'encoders': [
            ...
            # Flux encoder (unchanged)
            {'input': 70, 'hiddens':[128, 64], 'output': 64,
             'use_biases':[True, True, True], 'dropouts':[0.0, 0.0, 0.0],
        ...
        'decoders': [
            ...
            # Flux decoder (unchanged)
            {'input': 64, 'hiddens':[128], 'output': 70,
             'use_biases':[True, True], 'dropouts':[0.0, 0.0],
             'activations':['relu',None],
             'use_batch_norms':[False, False], 'use_layer_norms':[True, False]},
        ...
        'discriminators': [
            ...
            {'input': 70, 'hiddens':[128, 64], 'output': 1,
             'use_biases':[True]*3, 'dropouts':[0.1, 0.0, 0.0],
             'activations':['relu','relu','sigmoid'],
             'use_batch_norms':[False]*3, 'use_layer_norms':[False,False,True]},
            {'input': 1500, 'hiddens':[512, 256, 128], 'output': 1,
             'use_biases':[True]*4, 'dropouts':[0.1, 0.1, 0.0, 0.0],
             'activations':['relu','relu','relu','sigmoid'],
             'use_batch_norms':[False]*4, 'use_layer_norms':[False,False,False,True]},
...
def main():
    parser = argparse.ArgumentParser(
        description="Train UnitedNet PerturbMap - Activity + Flux + Niche (CapBoost v1)",
    ...
    # Model
    parser.add_argument('--technique', type=str, default='perturbmap_activity_flux_niche_capboost_v1',
                        choices=['perturbmap_activity_flux_niche_capboost_v1'])
    ...
    config['noise_level'] = [float(args.noise_level[0]), float(args.noise_level[1]), float(args.noise_level[2])]

    # Optional edge‑specific CMP weights
    if args.cmp_edge_activity_to_niche and args.cmp_edge_activity_to_niche > 0:
        # Use string key to keep config JSON-serializable; runtime consumers can parse "A->B" back to tuples if needed.
    ...
    # Ensure cmp_edge_weights is JSON-safe (string keys)
    ...
    for name, a, b in [("Activity", activity_tr, activity_te), ("Flux", flux_tr, flux_te), ("Niche", niche_tr, niche_te)]:
    ...
    # Train / load
    print(f"\n=== {'Training' if args.train_model else 'Loading'} UnitedNet (CapBoost) ===")
    ...
    # Save outputs
    model_fn = f"model_perturbmap_activity_flux_niche_capboost_v1_{args.dataset_id}"
    ...
    summary = {
        ...
        "scaling": {"flux": bool(args.scale_flux), "activity": bool(args.scale_activity), "niche": bool(args.scale_niche)},
    ...
    print("\n=== Notes ===")
    print("- CapBoost widens the Niche branch and reduces decoder dropout for more stable recon.")
    print("- Global CMP is higher; optionally bias Activity→Niche CMP if your build supports cmp_edge_weights.")
    print("- Enable --scale_niche (and --scale_activity) for better cross‑modal stability.")
