#!/usr/bin/env python3
"""
SHAP Analysis Script for UnitedNet PerturbMap CapBoost (RNA + Activity + Niche)
...
Usage:
    python calculate_shap_values_capboost_rna.py --dataset_id KP2_1
    python calculate_shap_values_capboost_rna.py --model_path ../Models/UnitedNet/perturbmap_rna_activity_niche_capboost_supervised_v2_20251128_123456/model_perturbmap_rna_activity_niche_capboost_supervised_v2_KP2_1_v2_improved_20251128_123456.pkl
"""
...
def find_latest_model(base_path="../Models/UnitedNet", dataset_id=None, technique="perturbmap_rna_activity_niche_capboost"):
    """Find the most recent model file based on timestamp"""
    search_patterns = []
    
    if dataset_id:
        # Look for specific dataset models (updated for RNA CapBoost naming)
        search_patterns.extend([
            ...
            f"{base_path}/*/model_perturbmap_rna_activity_niche_capboost_*_{dataset_id}_*.pkl",
        ])
    
    # Fallback patterns for RNA CapBoost models
    search_patterns.extend([
        f"{base_path}/{technique}_*/model_{technique}_*.pkl",
        f"{base_path}/{technique}/model_{technique}_*.pkl",
        f"{base_path}/*/model_perturbmap_rna_activity_niche_capboost_*.pkl",
        f"{base_path}/perturbmap_rna_activity_niche_capboost_*/model_*.pkl",
    ])
...
def load_model_and_data(model_path, data_path, dataset_id):
    ...
    # Load data files - updated for RNA+Activity+Niche modalities
    files_to_load = {
...
def prepare_data_for_shap(adata_dict):
    """
    Prepare data in the format needed for SHAP analysis - RNA+Activity+Niche
    FIXED: Properly handles phenotype extraction and NaN values
    """
    import pandas as pd
    import numpy as np
    
    # Extract individual datasets for RNA+Activity+Niche
    ...
    all_phenotypes = []
    for adata in [adata_rna_train, adata_activity_train, adata_niche_train, 
                  adata_rna_test, adata_activity_test, adata_niche_test]:
        if 'phenotypes' in adata.obs.columns:
    ...
    adata_rna_train = change_label_with_mapping(adata_rna_train, 'train', global_label_map)
    ...
    adata_rna_test = change_label_with_mapping(adata_rna_test, 'test', global_label_map)
    ...
    # Collect modalities into aligned lists - RNA+Activity+Niche order
    adatas_train = [adata_rna_train, adata_activity_train, adata_niche_train]
    adatas_test = [adata_rna_test, adata_activity_test, adata_niche_test]
...
def calculate_shap_values(model, adatas_all, technique="perturbmap_capboost_rna", output_dir="./"):
    """Calculate SHAP values for joint group identification - RNA+Activity+Niche"""
    
    # Define the modality names for RNA+Activity+Niche
    ...
    print(f"Expected dimensions: RNA(~1500-2000), Activity(~1500), Niche(~1500)")
    ...
    test_type = [
        ...
        torch.tensor(adatas_all[1].X, device=model.device, dtype=torch.float32),  # ACTIVITY
    ...
    try:
        ...
        try:
            ...
            try:
                    ...
                    for start_idx in range(0, test_type[0].shape[0], batch_size):
                        ...
                        try:
                            ...
                            if len(all_predictions) == 0:  # First batch, fit KMeans
                                n_clusters = 6  # Default assumption for RNA model
                ...
                from sklearn.cluster import KMeans
                n_clusters = 6  # Assuming 6 clusters based on RNA model setup
                kmeans = KMeans(n_clusters=n_clusters, random_state=42)
    ...
    try:
        ...
        # Use the first modality (RNA) as base and add UMAP if not present
...
def create_chord_plots(shap_values, predict_label, predict_label_anno, adata_fused, adatas_all, 
                      technique="perturbmap_capboost_rna", top_k_important=20, output_dir="./"):
    """Create chord plots for feature-to-group relevance analysis - RNA+Activity+Niche"""
    ...
    if "predicted_label_colors" not in adata_fused.uns.keys():
        sc.pl.umap(
            ...
            show=False,
            title=f"UMAP - {technique} Predicted Labels (RNA+Activity+Niche)",
            save=False
        )
    ...
    if 'label' in adata_fused.obs.columns:
        ...
        sc.pl.umap(
            ...
            palette="tab20",
            show=False,
            title=f"UMAP - {technique} Original Labels (RNA+Activity+Niche)",
            save=False
        )
    ...
    print("Note: RNA(~1500-2000) + Activity(~1500) + Niche(~1500) modalities")
    ...
    try:
        all_type_features, scores, aggregated_shap = type_relevance_chord_plot(
            ...
            colors_type,
            all_less_type,
            f"{technique} (RNA+Activity+Niche)",
            pr_ty_dict,
        ...
        if isinstance(shap_values, list) and len(shap_values) > 0:
            for cluster_idx in range(len(np.unique(predict_label))):
                ...
                for mod_idx, (mod_name, shap_mod) in enumerate(zip(["RNA", "Activity", "Niche"], shap_values)):
    ...
    print(f"Creating summary plots for RNA+Activity+Niche modalities...")
        ...
        try:
            if isinstance(aggregated_shap, dict):
                ...
                if all_scores:
                    ...
                    plt.title(f'Top {top_k_important} Features by Aggregated SHAP Score - {technique} (RNA+Activity+Niche)')
                    ...
                    plt.title(f'Top Features by Aggregated SHAP Score - {technique} (RNA+Activity+Niche)')
                    plt.savefig(plots_dir / f"top_features_barplot_{technique}.png", 
                               dpi=300, bbox_inches='tight', facecolor='white')
                ...
                if isinstance(aggregated_shap, np.ndarray) and aggregated_shap.size > 0:
                    ...
                    modality_dims = [adatas_all[i].X.shape[1] for i in range(3)]  # RNA, Activity, Niche
                    cumsum_dims = np.cumsum([0] + modality_dims)
                    
                    for idx in top_indices:
                        if idx < cumsum_dims[1]:
                            feature_names.append(f'RNA_{idx}')
                        elif idx < cumsum_dims[2]:
                            feature_names.append(f'Activity_{idx - cumsum_dims[1]}')
                        else:
    ...
    plt.title(f'Cluster Distribution - {technique} (RNA+Activity+Niche)')
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate SHAP values for UnitedNet CapBoost (RNA+Activity+Niche) joint group identification",
    ...
    parser.add_argument('--technique', type=str, default='perturbmap_capboost_rna',
    ...
    output_dir = base_output_dir / f"shap_analysis_capboost_rna_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"Results will be saved to: {output_dir}")
    print(f"Analyzing RNA CapBoost model with RNA(~1500-2000) + Activity(~1500) + Niche(~1500) modalities")
    ...
    try:
        ...
        if args.model_path and Path(args.model_path).exists():
            ...
            print("Searching for latest RNA CapBoost model...")
            model_path = find_latest_model(
                base_path=args.model_base_path,
                dataset_id=args.dataset_id,
                technique="perturbmap_rna_activity_niche_capboost"
            )
            if not model_path:
                raise FileNotFoundError(f"No RNA CapBoost model found for dataset {args.dataset_id}")
        
        # Load model and data
        model, adata_dict = load_model_and_data(model_path, args.data_path, args.dataset_id)
        
        # Prepare data
        print("\n=== Preparing Data (RNA + Activity + Niche) ===")
        ...
        if args.skip_shap and args.shap_file and Path(args.shap_file).exists():
            ...
            print(f"\n=== Calculating SHAP Values for RNA CapBoost Model ===")
        ...
        # Create chord plots
        print(f"\n=== Creating Chord Plots for RNA+Activity+Niche ===")
        ...
        print(f"Model analyzed: RNA CapBoost with RNA({adatas_all[0].X.shape[1]}) + "
              f"Activity({adatas_all[1].X.shape[1]}) + Niche({adatas_all[2].X.shape[1]}) features")
