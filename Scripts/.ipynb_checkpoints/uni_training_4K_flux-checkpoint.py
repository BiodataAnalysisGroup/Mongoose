#!/usr/bin/env python3
"""
UnitedNet PerturbMap Training Script with Flux Modality

This script trains a UnitedNet model for multi-modal biological data analysis
using the PerturbMap technique with RNA + Niche + Flux modalities. It supports 
various configurations and automatically handles data loading, preprocessing, and model training.
...
def load_adata_files(data_path, dataset_id):
    ...
    files_to_load = {
        ...
        'adata_flux_train': f'adata_flux_train_perturbmap_{dataset_id}.h5ad',  # CHANGED FROM ACTIVITY TO FLUX
        ...
        'adata_flux_test': f'adata_flux_test_perturbmap_{dataset_id}.h5ad'  # CHANGED FROM ACTIVITY TO FLUX
    }
...
def get_default_perturbmap_flux_config():
    """Returns the default PerturbMap configuration for RNA + Niche + Flux modalities"""
    return {
        'train_batch_size': 8,
        ...
        'encoders': [
            # RNA modality encoder
            {
                'input': 4001,  # UPDATED: Your actual RNA gene count
                'hiddens': [1024, 512, 256, 64],  # UPDATED: Larger network for more features
                'output': 64,
            ...
            # Niche modality encoder
            {
                'input': 4001,  # UPDATED: Your actual niche feature count
                'hiddens': [1024, 512, 256, 64],  # UPDATED: Larger network for more features
                'output': 64,
                'use_biases': [True, True, True, True, True],
                'dropouts': [0, 0, 0, 0, 0],
            ...
            # Flux modality encoder
            {
                'input': 70,  # UPDATED: Your actual metabolite count
                'hiddens': [256, 128, 64, 32],  # Appropriate for fewer features
                'output': 64,
                'use_biases': [True, True, True, True, True],
                'dropouts': [0, 0, 0, 0, 0],
                'activations': ['relu', 'relu', 'relu', 'relu', None],
        ...
        'decoders': [
            # RNA modality decoder
            {
                'input': 64,
                'hiddens': [64, 256, 512, 1024],  # UPDATED: Larger network for more features
                'output': 4001,  # UPDATED: Your actual RNA gene count
                'use_biases': [True, True, True, True, True],
                'dropouts': [0, 0, 0, 0, 0],
                'activations': ['relu', 'relu', 'relu', 'relu', None],
                'use_batch_norms': [False, False, False, False, False],
            ...
            # Niche modality decoder
            {
                'input': 64,
                'hiddens': [64, 256, 512, 1024],  # UPDATED: Larger network for more features
                'output': 4001,  # UPDATED: Your actual niche feature count
                'use_biases': [True, True, True, True, True],
                'dropouts': [0, 0, 0, 0, 0],
                'activations': ['relu', 'relu', 'relu', 'relu', None],
                'use_batch_norms': [False, False, False, False, False],
            ...
            # Flux modality decoder
            {
                'input': 64,
                'hiddens': [32, 64, 128, 256],  # Appropriate for fewer features
                'output': 70,  # UPDATED: Your actual metabolite count
                'use_biases': [True, True, True, True, True],
                'dropouts': [0, 0, 0, 0, 0],
                'activations': ['relu', 'relu', 'relu', 'relu', None],
                'use_batch_norms': [False, False, False, False, False],
        ...
        'discriminators': [
            # RNA modality discriminator
            {
                'input': 4001,  # UPDATED: Your actual RNA gene count
                'hiddens': [1024, 512, 256],  # UPDATED: Larger network for more features
                'output': 1,
            ...
            # Flux modality discriminator
            {
                'input': 70,  # UPDATED: Your actual metabolite count
                'hiddens': [256, 128, 64],  # Appropriate network for fewer features
                'output': 1,
                'use_biases': [True, True, True, True],
                'dropouts': [0, 0, 0, 0],
                'activations': ['relu', 'relu', 'relu', 'sigmoid'],
...
def load_config(config_path):
    ...
    if config_path.suffix == '.json':
        ...
        config_var_names = [
            'perturbmap_flux_config', 'perturbmap_config', 'perturbmap_config_corrected', 
...
def main():
    parser = argparse.ArgumentParser(
        description="Train UnitedNet PerturbMap model for multi-modal biological data with Flux",
    ...
    # Model arguments  
    parser.add_argument('--technique', type=str, default='perturbmap_flux',
                       choices=['perturbmap_flux'], help='Training technique')
    ...
    parser.add_argument('--noise_level', type=float, nargs=3, default=[0, 0, 0],
                       help='Noise levels for the 3 modalities (RNA, Niche, Flux)')
    ...
    if args.config_file and Path(args.config_file).exists():
        ...
        print("Using default PerturbMap Flux configuration")
        config = get_default_perturbmap_flux_config()
    ...
    try:
        ...
        adata_flux_train = adata_dict['adata_flux_train']  # CHANGED FROM ACTIVITY TO FLUX
        adata_rna_test = adata_dict['adata_rna_test']
        adata_niche_test = adata_dict['adata_niche_test']
        adata_flux_test = adata_dict['adata_flux_test']  # CHANGED FROM ACTIVITY TO FLUX
        
        print(f"Loaded training data shapes:")
        print(f"  RNA: {adata_rna_train.shape}")
        print(f"  Niche: {adata_niche_train.shape}")
        print(f"  Flux: {adata_flux_train.shape}")  # CHANGED FROM ACTIVITY TO FLUX
        
        # Print flux-specific information if available
        if 'metabolite_category' in adata_flux_train.var.columns:
            print(f"  Flux metabolite categories: {adata_flux_train.var['metabolite_category'].value_counts().to_dict()}")
        if 'has_flux_data' in adata_flux_train.obs.columns:
            print(f"  Cells with actual flux data: {adata_flux_train.obs['has_flux_data'].sum()}")
        
        # Add split labels
        print("\n=== Preprocessing Data ===")
        adata_rna_train = change_label(adata_rna_train, 'train')
        adata_niche_train = change_label(adata_niche_train, 'train')
        adata_flux_train = change_label(adata_flux_train, 'train')  # CHANGED FROM ACTIVITY TO FLUX
        adata_rna_test = change_label(adata_rna_test, 'test')
        adata_niche_test = change_label(adata_niche_test, 'test')
        adata_flux_test = change_label(adata_flux_test, 'test')  # CHANGED FROM ACTIVITY TO FLUX
        
        # Collect modalities into aligned lists
        adatas_train = [adata_rna_train, adata_niche_train, adata_flux_train]  # CHANGED
        adatas_test = [adata_rna_test, adata_niche_test, adata_flux_test]  # CHANGED
        ...
        modality_names = ['RNA', 'Niche', 'Flux']  # CHANGED
        for i, (ad, name) in enumerate(zip(adatas_train, modality_names)):
            in_dim = ad.X.shape[1]
            config["encoders"][i]["input"] = in_dim
            config["decoders"][i]["output"] = in_dim  
            config["discriminators"][i]["input"] = in_dim
            print(f"{name} modality: {in_dim} features")
        ...
        if args.save_intermediates:
            ...
            for i, (name, adata) in enumerate(zip(['rna', 'niche', 'flux'], adatas_train)):  # CHANGED
                adata.write(output_dir / f"preprocessed_{name}_train.h5ad")
            for i, (name, adata) in enumerate(zip(['rna', 'niche', 'flux'], adatas_all)):  # CHANGED
        ...
        model_filename = f"model_perturbmap_flux_{args.dataset_id}"  # CHANGED
        ...
        summary = {
            ...
            'modalities': ['RNA', 'Niche', 'Flux'],  # CHANGED
            'device': args.device,
            'timestamp': timestamp,
            ...
            'data_shapes': {
                ...
                'flux': list(adata_flux_train.shape)  # CHANGED
            },
        ...
        print(f"Output directory: {output_dir}")
        print(f"Modalities trained: RNA + Niche + Flux")
        
        # Print flux-specific summary
        if 'metabolite_category' in adata_flux_train.var.columns:
            print(f"Flux modality metabolite categories: {adata_flux_train.var['metabolite_category'].value_counts().to_dict()}")
