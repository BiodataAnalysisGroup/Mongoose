#!/usr/bin/env python3
"""
Activity Matrix Integration Script with Force-Include Support
...
Supports force-including specific activities in HVG selection.
"""
...
def calculate_highly_variable_activities(adata, n_hvg=2000, normalize=False, force_include_activities=None, logger=None):
    """Calculate highly variable activities and subset with optional force-include."""
    if logger is None:
        logger = logging.getLogger(__name__)
    ...
    # Save original HVG mask
    hvg_mask = adata_hvg.var["highly_variable"].copy()
    logger.info(f"HVGs detected: {hvg_mask.sum()}")
    
    # Force-include specific activities
    if force_include_activities:
        hvg_mask_forced = hvg_mask.copy()
        for activity in force_include_activities:
            if activity in adata_hvg.var_names:
                hvg_mask_forced.loc[activity] = True
                logger.info(f"Force-included activity: {activity}")
            else:
                logger.warning(f"Activity '{activity}' not found in activity matrix")
                logger.info(f"Available activities containing '{activity}': {[var for var in adata_hvg.var_names if activity.lower() in var.lower()]}")
        
        logger.info(f"Total selected activities (HVG + forced): {hvg_mask_forced.sum()}")
        hvg_activities = adata_hvg.var_names[hvg_mask_forced].tolist()
    else:
        hvg_activities = adata_hvg.var_names[hvg_mask].tolist()
    
    # Subset to selected activities
    ...
    adata_hvg_subset.uns["hvg_calculation"] = {
        ...
        "force_included": force_include_activities,
...
def list_available_activities(activity_matrix_path, search_term=None, logger=None):
    """List available activities in the matrix, optionally filtering by search term."""
    if logger is None:
        logger = logging.getLogger(__name__)
    
    logger.info(f"Examining activity matrix: {activity_matrix_path}")
    
    # Load just the header to get activity names
    activity_df = pd.read_csv(activity_matrix_path, index_col=0, nrows=0)
    activities = list(activity_df.columns)
    
    logger.info(f"Total activities available: {len(activities)}")
    
    if search_term:
        matching_activities = [act for act in activities if search_term.lower() in act.lower()]
        logger.info(f"Activities matching '{search_term}': {len(matching_activities)}")
        for activity in matching_activities:
            logger.info(f"  - {activity}")
    else:
        logger.info("First 20 activities:")
        for activity in activities[:20]:
            logger.info(f"  - {activity}")
        if len(activities) > 20:
            logger.info(f"  ... and {len(activities) - 20} more")

def main():
    parser = argparse.ArgumentParser(
        description="Integrate scMINER activity matrix with AnnData object (with force-include support)",
    ...
    parser.add_argument("--search-activities", 
                       help="Search for activities containing this term")
    
    # HVG parameters
    parser.add_argument("--calculate-hvg", action="store_true",
                       help="Calculate highly variable activities")
    parser.add_argument("--n-hvg", type=int, default=2000,
                       help="Number of highly variable activities to select")
    parser.add_argument("--normalize-for-hvg", action="store_true",
                       help="Normalize data before HVG calculation")
    parser.add_argument("--force-include-activities", default="",
                       help="Comma-separated list of activities to force-include in HVG selection")
    ...
    logger.info("Starting activity matrix integration with force-include support")
    ...
    try:
        ...
        # Handle activity listing mode
        if args.list_activities:
            list_available_activities(args.activity_matrix, args.search_activities, logger)
            return
        
        # For normal processing, we need input-adata and output
        if not args.input_adata:
            raise ValueError("--input-adata is required for normal processing")
        if not args.output:
            raise ValueError("--output is required for normal processing")
        ...
        if args.calculate_hvg:
            # Parse force-include activities
            force_include_activities = [x.strip() for x in args.force_include_activities.split(",") if x.strip()] if args.force_include_activities else None
            
            new_adata, hvg_activities = calculate_highly_variable_activities(
                new_adata, 
                args.n_hvg, 
                args.normalize_for_hvg,
                force_include_activities,
                logger
            )
            logger.info(f"Selected {len(hvg_activities)} highly variable activities")
            
            if force_include_activities:
                logger.info(f"Requested force-include activities: {force_include_activities}")
        ...
        if args.calculate_hvg:
            ...
            if force_include_activities:
                logger.info(f"  Force-included activities: {force_include_activities}")
