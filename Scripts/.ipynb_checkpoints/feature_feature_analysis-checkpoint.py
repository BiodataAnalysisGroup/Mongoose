#!/usr/bin/env python3
"""
Feature-to-Feature Cross-Modal Prediction Analysis for UnitedNet
...
Usage:
    python feature_feature_analysis.py --dataset_id KP2_1 --shap_dir ../Task1_JGI/shap_analysis_20250911_143045
"""
...
def load_shap_results(shap_dir):
    ...
    # Load the main SHAP results with exact names
    results = {}
    files_to_load = {
        'all_type_features': 'all_type_features_perturbmap.pkl',
        'scores': 'scores_perturbmap.pkl', 
        'aggregated_shap': 'aggregated_shap_perturbmap.pkl',
        'shap_values': 'shap_values_perturbmap.pkl'
    }
...
def prepare_data_for_cross_modal(data_path, dataset_id):
    ...
    # Process data
    for key in ['adata_rna_train', 'adata_niche_train', 'adata_activity_train']:
        adata_dict[key] = change_label(adata_dict[key], 'train')
    for key in ['adata_rna_test', 'adata_niche_test', 'adata_activity_test']:
...
def str_to_mod_id(x):
    ...
    if s in ("2", "activity", "act", "a"): return 2
    try: return int(s)
...
def feature_feature_relevance(shap_values_dict, all_type_features, var_names_all, 
                            source_id, target_id, technique="perturbmap", make_plot=False):
...
def extract_feature_feature_values(shap_values_dict, all_type_features, var_names_all, 
                                 combinations, technique="perturbmap"):
    """Extract feature-feature values for all combinations"""
    
    id2modality = {0: "RNA", 1: "NICHE", 2: "ACTIVITY"}
    rows = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Feature-to-feature cross-modal prediction analysis for UnitedNet",
    ...
    parser.add_argument('--shap_dir', type=str, required=True,
                       help='Directory containing SHAP analysis results from previous step')
    ...
    if args.timestamp:
        ...
        output_dir = Path(args.output_dir) / f"cross_modal_analysis_{timestamp}"
        ...
        # No timestamp
        timestamp = "manual"
        output_dir = Path(args.output_dir) / "cross_modal_analysis"
    ...
    try:
        ...
        for i, var_names in enumerate(var_names_all):
            print(f"  Modality {i}: {len(var_names)} features")
        ...
        # Save results
        feature_feature_file = output_dir / 'feature_feature_importance_3modalities.csv'
