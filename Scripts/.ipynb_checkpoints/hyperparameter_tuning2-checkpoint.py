...
class HyperparameterTuner:
    ...
    def _train_model(self, param_name: str, param_value: Any, exp_dir: Path) -> Path:
        ...
        if param_name == 'learning_rate' or param_name == 'lr':
            ...
            # This would require modifying uni_training.py to accept latent_dim
            cmd.extend(["--latent_dim", str(param_value)])
        elif param_name == 'noise_level':
            # Handle noise_level as comma-separated values
            if isinstance(param_value, str):
                cmd.extend(["--noise_level"] + param_value.split(','))
            else:
                cmd.extend(["--noise_level", str(param_value), str(param_value), str(param_value)])
        elif param_name == 'encoder_depth':
            # This would require modifying uni_training.py to accept encoder_depth
            cmd.extend(["--encoder_depth", str(param_value)])
