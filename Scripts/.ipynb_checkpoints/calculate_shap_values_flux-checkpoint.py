#!/usr/bin/env python3
"""
SHAP Analysis Script for UnitedNet PerturbMap with Flux Modality
...
Usage:
    ...
    python calculate_shap_values.py --model_path ../Model/perturbmap_flux/model_perturbmap_flux_KP2_1.pkl
"""
...
def find_latest_model(base_path="../Model", dataset_id=None, technique="perturbmap_flux"):
    """Find the most recent model file based on timestamp"""
    search_patterns = []
    
    if dataset_id:
        # Look for specific dataset models with flux
        search_patterns.extend([
            ...
            # Also search for generic perturbmap models
...
def calculate_shap_values(model, adatas_all, technique="perturbmap_flux", output_dir="./"):
    """Calculate SHAP values for joint group identification"""
    
    # Define the modality names for all 3 modalities - CHANGED FROM ACTIVITY TO FLUX
    ...
    if 'metabolite_category' in adatas_all[2].var.columns:
        print(f"Flux metabolite categories: {adatas_all[2].var['metabolite_category'].value_counts().to_dict()}")
    if 'has_flux_data' in adatas_all[2].obs.columns:
        print(f"Cells with actual flux data: {adatas_all[2].obs['has_flux_data'].sum()}")
    ...
    test_type = [
        ...
        torch.tensor(adatas_all[2].X, device=model.device, dtype=torch.float32),  # FLUX (CHANGED)
    ]
...
def create_chord_plots(shap_values, predict_label, predict_label_anno, adata_fused, adatas_all, 
                      technique="perturbmap_flux", top_k_important=20, output_dir="./"):
    ...
    print(f"Feature breakdown by modality:")
    modality_names = ["RNA", "NICHE", "FLUX"]  # CHANGED
    for i, (name, adata) in enumerate(zip(modality_names, adatas_all)):
        print(f"  {name}: {adata.n_vars} features")
        if name == "FLUX" and 'metabolite_category' in adata.var.columns:
            print(f"    Metabolite categories: {adata.var['metabolite_category'].value_counts().to_dict()}")
                ...
                if isinstance(aggregated_shap, np.ndarray) and aggregated_shap.size > 0:
                    ...
                    for i, (name, adata) in enumerate(zip(modality_names, adatas_all)):
                        ...
                        for idx in top_indices:
                            if cumulative_features <= idx < cumulative_features + n_features:
                                local_idx = idx - cumulative_features
                                if name == "FLUX" and len(adata.var_names) > local_idx:
                                    # For flux, use metabolite names
                                    metabolite_name = adata.var_names[local_idx]
                                    feature_names.append(f"{name}:{metabolite_name}")
                                else:
                                    feature_names.append(f"{name}:Feature_{local_idx}")
                                break
                        cumulative_features += n_features
                    
                    # If we didn't get enough names, fill with generic ones
                    while len(feature_names) < len(top_indices):
                        feature_names.append(f'Feature_{len(feature_names)}')
                    
                    feature_names = feature_names[:len(top_scores)]  # Match lengths
    ...
    # Create metabolite-specific summary for flux modality
    if len(adatas_all) > 2 and 'metabolite_category' in adatas_all[2].var.columns:
        plt.figure(figsize=(12, 6))
        category_counts = adatas_all[2].var['metabolite_category'].value_counts()
        plt.bar(range(len(category_counts)), category_counts.values)
        plt.xlabel('Metabolite Category')
        plt.ylabel('Number of Metabolites')
        plt.title(f'Flux Modality - Metabolite Categories Distribution')
        plt.xticks(range(len(category_counts)), category_counts.index, rotation=45)
        plt.tight_layout()
        plt.savefig(plots_dir / f"metabolite_categories_{technique}.png", 
                   dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate SHAP values for UnitedNet joint group identification with Flux modality",
    ...
    output_dir = base_output_dir / f"shap_analysis_flux_{timestamp}"
    ...
    try:
        ...
        for i, (name, adata) in enumerate(zip(modality_names, adatas_all)):
            ...
            if name == "FLUX":
                if 'metabolite_category' in adata.var.columns:
                    print(f"    Metabolite categories: {adata.var['metabolite_category'].value_counts().to_dict()}")
                if 'has_flux_data' in adata.obs.columns:
                    print(f"    Cells with actual flux data: {adata.obs['has_flux_data'].sum()}")
        ...
        print(f"Analyzed modalities: RNA + Niche + Flux")
        
        # Print flux-specific summary
        if len(adatas_all) > 2:
            flux_data = adatas_all[2]
            print(f"Flux modality summary:")
            print(f"  - Metabolites analyzed: {flux_data.n_vars}")
            if 'metabolite_category' in flux_data.var.columns:
                print(f"  - Metabolite categories: {flux_data.var['metabolite_category'].value_counts().to_dict()}")
